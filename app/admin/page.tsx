...
export default function AdminDashboard() {
    ...
    const totalBukuDipinjam = peminjamanList.reduce(
        (total, pinjam) => total + pinjam.detail.length, 0
    );

    // TOTAL TRANSAKSI PEMINJAMAN
    const totalBelumDikembalikan = peminjamanList.reduce(
        (total, pinjam) => total + pinjam.detail.filter(
                (detail) => detail.status === "dipinjam").length, 0
    );

    // JUMLAH BUKU YANG TERLAMBAT DIKEMBALIKAN
    const totalTerlambat = peminjamanList.reduce(
        (total, pinjam) => total + pinjam.detail.filter(
                (detail) => detail.status === "terlambat").length, 0
    );
    ...
    return (
        ...
        <div className={`${inter.className} space-y-6`}>
            ...
            <div className="bg-white rounded-xl md:rounded-2xl shadow-sm p-4 md:p-6 min-h-[300px]">
